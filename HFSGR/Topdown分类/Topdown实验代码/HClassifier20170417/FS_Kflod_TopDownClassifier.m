...
%  indices = varargin{4};
%% Output
function [accuracyMean,accuracyStd,F_LCAMean,FHMean] = FS_Kflod_TopDownClassifier(data, numFolds, tree,flag, feature, numberSel,indices)
...
for k = 1:numFolds
    ...
    for i = 1:numNodes
        if (~ismember(i, tree_LeafNode(tree)))
            A=[79,80];
            if (~ismember(i,A))%此条件只针对CLEF，因为它中有空矩阵
    ...
    %%           Prediction       %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if (flag==1)
    [predict_label] = FS_topDownPrediction(test_data, model, tree,feature,numberSel) ;
    else 
